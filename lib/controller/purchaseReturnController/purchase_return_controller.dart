...
import 'package:accountech/model/purchaseReturnModel/purchase_return_list_model.dart';
...
class PurchaseReturnController extends GetxController {
  ...
  var purchaseReturnList = <Data>[].obs;
  ...
  Future<dynamic> getPurchaseReturnList() async {
    Uri uri = Uri.parse(endpoint.getPurchaseReturn +
        MySharedPreferences.businessSelectedID.$.toString());
    var response = await http.get(uri);
    try {
      var model = PurchaseReturnListModel.fromJson(json.decode(response.body));
      purchaseReturnList.value = model.data!;
      return model;
    } catch (e) {
      return "";
    }
  ...
  Future<dynamic> deletePurchaseReturn(String id) async {
    debugPrint("id is ${endpoint.getPurchaseReturn}$id");
    Uri uri = Uri.parse(endpoint.getPurchaseReturn + id.toString());
