...
import 'package:accountech/model/deliveryChallanModel/delivery_challan_list_model.dart';
...
class DeliveryChallanController extends GetxController {
  ...
  var deliveryChallanList = <Data>[].obs;
  var isLoading = false.obs;


  var date ="00-00-0000".obs;
  var validityDate ="00-00-0000".obs;
  var deliveryChallanNo ="".obs;


  resetVariables(){
    date ="00-00-0000".obs;
    validityDate ="00-00-0000".obs;
    deliveryChallanNo ="".obs;
  }


  Future<dynamic> getDeliveryChallanList() async {
    Uri uri = Uri.parse(endpoint.getDebitNote +
        MySharedPreferences.businessSelectedID.$.toString());
    var response = await http.get(uri);
    try {
      var model = DeliveryChallanListModel.fromJson(json.decode(response.body));
      deliveryChallanList.value = model.data!;
      return model;
    } catch (e) {
      return "";
    }
  ...
  Future<dynamic> deleteDeliveryChallan(String id) async {
    debugPrint("id is ${endpoint.getDebitNote}$id");
    Uri uri = Uri.parse(endpoint.getDebitNote + id.toString());
