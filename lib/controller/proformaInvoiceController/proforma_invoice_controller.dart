...
import 'package:accountech/model/proformaInvoiceModel/proforma_invoice_list_model.dart';
...
class ProformaInvoiceController extends GetxController {
  var endpoint = ApiServices();
  var proformaInvoiceList = <Data>[].obs;
  var isLoading = false.obs;


  var date ="00-00-0000".obs;
  var validityDate ="00-00-0000".obs;
  var proformaInvoiceNo ="".obs;


  resetVariables(){
    date ="00-00-0000".obs;
    validityDate ="00-00-0000".obs;
    proformaInvoiceNo ="".obs;
  }


  Future<dynamic> getProformaInvoiceList() async {
    Uri uri = Uri.parse(endpoint.getProformaInvoice +
        MySharedPreferences.businessSelectedID.$.toString());
    var response = await http.get(uri);
    try {
      var model = ProformaInvoiceListModel.fromJson(json.decode(response.body));
      proformaInvoiceList.value = model.data!;
      return model;
    } catch (e) {
      return "";
    }
  ...
  Future<dynamic> deleteProformaInvoice(String id) async {
    debugPrint("id is ${endpoint.deleteProformaInvoice}$id");
    Uri uri = Uri.parse(endpoint.deleteProformaInvoice + id.toString());
