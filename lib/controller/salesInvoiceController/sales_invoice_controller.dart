...
import 'package:accountech/model/salesInvoiceModel/sales_invoice_list_model.dart';
...
class SalesInvoiceController extends GetxController {
  ...
  Future<dynamic> getSalesInvoiceList() async {
    Uri uri = Uri.parse(endpoint.getSalesInvoice +
        MySharedPreferences.businessSelectedID.$.toString());
    var response = await http.get(uri);
    try {
      var model = SalesInvoiceListModel.fromJson(json.decode(response.body));
      salesInvoiceList.value = model.data!;
      return model;
    } catch (e) {
      return "";
    }
  ...
  Future<dynamic> deleteSaleInvoice(String id) async {
    debugPrint("id is ${endpoint.deleteSalesInvoice}$id");
    Uri uri = Uri.parse(endpoint.deleteSalesInvoice + id.toString());
